        ...
        <Stack horizontal className={styles.chatRoot}>
          <div className={styles.chatContainer}>
            {!messages || messages.length < 1 ? (
              <Stack className={styles.chatEmptyState}>
                <img src={ui?.chat_logo ? ui.chat_logo : SSbanner} className={styles.chatIcon} aria-hidden="true" />
                <h1 className={styles.chatEmptyStateTitle}>{ui?.chat_title}</h1>
                <h2 className={styles.chatEmptyStateSubtitle}>{ui?.chat_description}</h2>
                <div className={styles.chatButtonGroup}>
                  {ui?.conversation_starter_1 ? <button className={styles.chatButton} onClick={() => setQuickSend(ui.conversation_starter_1 ?? "")}>{ui.conversation_starter_1}</button> : null}
                  {ui?.conversation_starter_2 ? <button className={styles.chatButton} onClick={() => setQuickSend(ui.conversation_starter_2 ?? "")}>{ui.conversation_starter_2}</button> : null}
                  {ui?.conversation_starter_3 ? <button className={styles.chatButton} onClick={() => setQuickSend(ui.conversation_starter_3 ?? "")}>{ui.conversation_starter_3}</button> : null}
                  {ui?.conversation_starter_4 ? <button className={styles.chatButton} onClick={() => setQuickSend(ui.conversation_starter_4 ?? "")}>{ui.conversation_starter_4}</button> : null}
                </div>
              </Stack>
            ) : (
